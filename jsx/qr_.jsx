...
// Illustrator QR Generator
///////////////////////////////////
Array.prototype.isEqualTo = function(arr) {
    var key;
    if (this.length !== arr.length) return false;
    for (key in this) {
        if (this[key] !== arr[key]) return false;
    }
    return true;
};
...
Array.prototype.has = function(ele) {
    ...
    for (i; i < length; i += 1) {
        if (typeof(ele) === 'object' && typeof(this[i]) === 'object') {
            if (this[i].isEqualTo(ele)) return true;
        } else {
            if (ele === this[i]) return true;
        }
    }
    return false;
};
...
var IllustratorQR = function() {
    ...
    function isCaptured(block, origBlock, direction) {
        ...
        if (c < 0 || r < 0 || c > (gridCols - 1) || r > (gridRows - 1)) return false;
        ...
        if (direction === 'left') {
            if (isCaptured([r, c - 1], origBlock)) return true;
            if (isCaptured([r - 1, c], origBlock)) return true;
        } else {
            if (isCaptured([r, c + 1], origBlock, 'right')) return true;
            if (isCaptured([r + 1, c], origBlock, 'right')) return true;
        }
    ...
    function getNextPosition(polygon) {
        ...
        neighbors[0] = [lineCursor[0], lineCursor[1] - 1];
        neighbors[1] = [lineCursor[0], lineCursor[1]];
        neighbors[2] = [lineCursor[0] - 1, lineCursor[1]];
        neighbors[3] = [lineCursor[0] - 1, lineCursor[1] - 1];
    ...
    function getIllustratorPoints(points, size) {
        ...
        for (p; p < numPoints; p += 1) {
            ...
            illPoints.push([
                (points[p][0] * size),
                ((points[p][1] * size * -1) + illDocument.height)
            ]);
        }
        illPoints.pop();
        return illPoints;
 ...
 */
function generateQRCode(text, x, y, width, height, textPadding, layer) {
    try {
        // Validate input
        ...
        // Initialize Illustrator QR generator
        var IllQR = new IllustratorQR();
        
        // Create a temporary document to generate the QR code
        var tempDoc = app.documents.add(DocumentColorSpace.CMYK, qr.modules.length, qr.modules.length);
        IllQR.init(qr.modules, tempDoc);
        
        // Calculate module size (1pt per module in temp doc)
        var moduleSize = 1;
        IllQR.make(moduleSize);
        
        // Get the QR code group from the temporary document
        var qrGroup = tempDoc.groupItems[0];
        
        // Copy the QR code to the target document
        qrGroup.selected = true;
        app.copy();
        tempDoc.close(SaveOptions.DONOTSAVECHANGES);
        
        // Paste into the target layer
        var pastedItems = layer.paste();
        if (pastedItems.length === 0) throw new Error("Failed to paste QR code");
        
        // Get the pasted group
        var qrArt = pastedItems[0];
        if (qrArt.typename !== "GroupItem") {
            // If it's not a group, create one and put it inside
            var wrapperGroup = layer.groupItems.add();
            qrArt.move(wrapperGroup, ElementPlacement.PLACEATBEGINNING);
            qrArt = wrapperGroup;
        }
        
        // Scale and position the QR code
        var bounds = qrArt.geometricBounds;
        var currentWidth = bounds[2] - bounds[0];
        var scale = width / currentWidth;
        
        qrArt.resize(scale * 100, scale * 100);
        qrArt.position = [x, y - height]; // Adjust position to match EAN barcode behavior
        
        // Add white background (like EAN barcode)
        var bgRect = layer.pathItems.rectangle(y, x, width, height);
        bgRect.stroked = false;
        bgRect.filled = true;
        bgRect.fillColor = make_cmyk(0, 0, 0, 0); // White
        bgRect.move(qrArt, ElementPlacement.PLACEBEHIND);
        
        // Add text below (like EAN barcode)
        var fontSize = Math.min(12, height * 0.15);
        var textItem = layer.textFrames.pointText([x + width/2, y - height - textPadding]);
        textItem.contents = text;
        textItem.textRange.characterAttributes.size = fontSize;
        textItem.textRange.characterAttributes.textFont = textFonts.getByName("ArialMT");
        textItem.textRange.paragraphs.justification = Justification.CENTER;
        
        // Group everything together
        var finalGroup = layer.groupItems.add();
        finalGroup.move(layer, ElementPlacement.PLACEATEND);
        qrArt.move(finalGroup, ElementPlacement.INSIDE);
        bgRect.move(finalGroup, ElementPlacement.INSIDE);
        textItem.move(finalGroup, ElementPlacement.INSIDE);
 ...
 * 
 * // Generate a QR code
 * var qrGroup = generateQRCode(
 *     "https://example.com",  // text
 *     100,                    // x position
 *     200,                    // y position
 *     144,                    // width (2 inches at 72ppi)
 *     144,                    // height
 *     20,                     // text padding
 *     layer                   // target layer
 * );
